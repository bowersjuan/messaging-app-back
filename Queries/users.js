...
const getAllMessagesForUser = async (id) => {
  try {
    const messagesByUser = await db.any(
      'SELECT message_id, user_id, sender_id, message, favorite, time_sent FROM users_messages JOIN users ON users.id = users_messages.user_id JOIN messages ON messages.id = users_messages.message_id WHERE users_messages.user_id = $1',
      id
    );
...
const getOneMessagesForUser = async (id) => {
  try {
    const messageByUser = await db.one(
      'SELECT sender_id, message, favorite, time_sent FROM messages WHERE messages.id = $1',
      id
    );
